...
InjectClipboard(hk := "") {
    ...
    ; 等待物理修饰键（Ctrl/Shift/Alt/Win）彻底释放，防止与开头字符组合成快捷键被目标窗口吞掉
...
ShowGui(*) {
    ...
    MyGui := Gui(, AppName . " - " . Lang["Tray_Settings"])
    MyGui.Opt("+AlwaysOnTop")
    MyGui.SetFont("s10", "Segoe UI, Microsoft YaHei")

    MyGui.Add("Text", "xm y+10", Lang["Gui_Lang"])
    LangOptions := ["English", "العربية", "简体中文"]
    LangMap := ["EN", "AR", "ZH"]
    LangChoice := (CurrentLang = "ZH") ? 3 : ((CurrentLang = "AR") ? 2 : 1)
    Ctrl_Lang := MyGui.Add("DropDownList", "w320 Choose" . LangChoice, LangOptions)
    Ctrl_Lang.OnEvent("Change", (*) => ChangeLang(LangMap[Ctrl_Lang.Value]))
...
UpdateHotkey(NewHk, ShowError := True) {
    try {
        ...
        if (ShowError) {
            ...
            if (!WinExist(AppName . " - " . Lang["Tray_Settings"]) && !WinExist(AppName . " - Settings")) {
                ShowGui()
            }
        }
        return False
...
LoadSettings() {
    Global
    DefaultLang := (A_Language = "0804" || A_Language = "1004" || A_Language = "0404" || A_Language = "0c04") ? "ZH" : "EN"
    CurrentLang := IniRead(IniFile, "Settings", "Language", DefaultLang)
...
SetupLanguage() {
    if (CurrentLang = "ZH") {
        Lang["Tray_TypeNow"] := "立即模拟输入剪贴板"
        Lang["Tray_Settings"] := "设置"
        Lang["Tray_Pause"] := "暂停 / 恢复"
        Lang["Tray_Exit"] := "退出"
        Lang["Gui_Lang"] := "界面语言 / Language:"
        Lang["Gui_Delay"] := "基础打字延迟 (毫秒):"
        Lang["Gui_MaxDelay"] := "最大打字延迟 (用于随机延迟):"
        Lang["Gui_Hotkey"] := "快捷键:"
        Lang["Gui_Random"] := "启用随机打字延迟 (防机器人/绕过反作弊检测)"
        Lang["Gui_Smart"] := "标点符号智能停顿 (模拟人类打字停顿)"
        Lang["Gui_Trim"] := "自动去除首尾空白字符"
        Lang["Gui_Wipe"] := "安全擦除 (输入完成后立即清空剪贴板)"
        Lang["Gui_Admin"] := "以管理员身份运行 (支持向高权限窗口输入)"
        Lang["Gui_Startup"] := "开机自启动 (注册表)"
        Lang["Gui_Sound"] := "启用音效提示"
        Lang["Btn_Save"] := "保存(&S)"
        Lang["Btn_Cancel"] := "取消(&C)"
        Lang["Msg_InvalidHk"] := "请设置有效的快捷键！"
        Lang["Msg_HkFail"] := "快捷键注册失败！可能已被其他程序占用。"
        Lang["Msg_AdminFail"] := "以管理员身份运行失败。ClipType 将以普通权限运行，但无法向管理员权限窗口输入内容。"
        Lang["Msg_RestartAdmin"] := "设置已保存。请重新启动程序以应用管理员权限。"
        Lang["Msg_AdminStartupWarn"] := "警告：同时启用“以管理员身份运行”与“开机自启动”，每次开机登录时都会触发 Windows UAC 提示。"
        Lang["Status_Paused"] := "已暂停"
        Lang["Status_Ready"] := "就绪"
